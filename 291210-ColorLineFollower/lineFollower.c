	...
	if (line_side == 0) {
		...
		switch(direction) {
			...
			case FOLLOW:
				...
				if (old_state != BLACK || old_state != UNSURE) {
					motor_right = 25;
					motor_left = 28;
				}
...
void check_color() {
	...
	/* Hold state in color until line is found */
	if (old_state == YELLOW) {
...
TASK (ManageState) {
	...
	switch (state) {
		...
		case SEARCH_CORNER:
			...
			else if (search_time < 400) {

				set_both_motor_speed(20,-55);
			...
			else if (search_time < 800) { 
				
				set_both_motor_speed(-55,20); 
		...
		case CALIBRATE:
			...
			if (color_cal_state < NUM_COLORS) {
				...
				if (enter_button == 3) {
					...
					colorCal[color_cal_state][0] = (rgb[0]-70);
					colorCal[color_cal_state][1] = (rgb[0]+70);
					colorCal[color_cal_state][2] = (rgb[1]-70);
					colorCal[color_cal_state][3] = (rgb[1]+70);
					colorCal[color_cal_state][4] = (rgb[2]-70);
					colorCal[color_cal_state][5] = (rgb[2]+70);
