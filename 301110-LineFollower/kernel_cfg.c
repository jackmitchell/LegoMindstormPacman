/*
 *  kernel_cfg.c
 *  Wed Nov 24 17:48:17 2010
 *  SG Version 2.00
 *  sg.exe ./lineFollower.oil -os=ECC2 -I../../ecrobot/../toppers_osek/sg/impl_oil -template=../../ecrobot/../toppers_osek/sg/lego_nxt.sgt
 */
...
const TaskType TaskLCD = 0;
const TaskType TaskColour = 1;

extern void TASKNAME( TaskLCD )( void );
extern void TASKNAME( TaskColour )( void );

static __STK_UNIT _stack_TaskLCD[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_TaskColour[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 2, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 2, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( TaskLCD ), TASKNAME( TaskColour ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_TaskLCD, (__STK_UNIT)_stack_TaskColour, };
...
const AlarmType cyclic_alarm_TaskColour = 0;
const AlarmType cyclic_alarm_TaskLCD = 1;

DeclareTask(TaskColour);
static void _activate_alarm_cyclic_alarm_TaskColour( void );
static void _activate_alarm_cyclic_alarm_TaskColour( void )
{ (void)ActivateTask( TaskColour ); }

DeclareTask(TaskLCD);
static void _activate_alarm_cyclic_alarm_TaskLCD( void );
static void _activate_alarm_cyclic_alarm_TaskLCD( void )
{ (void)ActivateTask( TaskLCD ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_cyclic_alarm_TaskColour, _activate_alarm_cyclic_alarm_TaskLCD, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 1, 500, };
